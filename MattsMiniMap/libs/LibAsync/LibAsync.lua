local MAJOR, MINOR = "LibAsync", 2.1
local async, oldminor = LibStub and LibStub:NewLibrary(MAJOR, MINOR)
if LibStub and not async then
	return
...
function async.Scheduler()
	...
	job = nil
	local start, runTime, name = GetFrameTimeSeconds()
	runTime, cpuLoad = start, GetGameTimeSeconds() - start
	if cpuLoad > spendTime then
		spendTime = min(0.030, spendTime + spendTime * 0.02)
		if debug then
			df("initial gap: %ims. skip. new threshold: %ims", GetGameTimeSeconds() - start, spendTime)
		end
		return
	end
	if debug then
		df("initial gap: %ims", (GetGameTimeSeconds() - start) * 1000)
	end
	while (GetGameTimeSeconds() - start) <= spendTime do
		name, job = next(jobs, name)
		if not job then
			name, job = next(jobs)
		end
		if job then
			runTime = GetGameTimeSeconds()
			DoJob(job)
		else
			-- Finished
			running = false
	...
	spendTime = max(GetThreshold(), spendTime - spendTime * 0.01)
	if debug and job then
		local now = GetGameTimeSeconds()
		local freezeTime = now - start
		if freezeTime >= 0.016 then
			runTime = now - runTime
			df("%s freeze. allowed: %ims, used %ims, resulting fps %i.", job.name, spendTime * 1000, runTime * 1000, 1 / freezeTime)
		end
	end
end
